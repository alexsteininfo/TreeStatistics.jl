...
@testset "simulate till branching" begin
    @test moduletracker.tvec[end] <= pop_age
...
@testset "simulate to fixed size" begin
    populationtracker = SomaticEvolution.initialize_population(input.modulesize, clonalmutations=0)
    ...
    populationtracker, mutID, t = 
        SomaticEvolution.update_population!(populationtracker, input, mutID, 0, rng)
    #popsize is now 2
    ...
    #check only homeostatic modules are chosen for Moran and branching and that prob of choosing cells is equal
